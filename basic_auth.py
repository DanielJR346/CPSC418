...
def safe_prime( bits:int=512 ) -> int:
    ...
    # delete this comment and insert your code here
    """
    x = b'\1' + int_to_bytes(secrets.randbits(bits-1), math.ceil((bits-1)/8))
    while (not sympy.isprime(bytes_to_int(x))) and (not sympy.isprime (2 * bytes_to_int(x) + 1)):
        x = b'\1' + int_to_bytes(secrets.randbits(bits-1), math.ceil((bits-1)/8))
    ...
    safePrime = secrets.randbits(bits)

    print("for testing ",bits) 
    """
    while (not sympy.isprime(safePrime)) and (not sympy.isprime((2 * safePrime) + 1)) and (dead & safePrime != dead):
        safePrime = secrets.randbits(bits)
    """
    """
    while True:
        safePrime = secrets.randbits(bits)
        if (sympy.isprime(int(safePrime))):
            if sympy.isprime( int((2 * safePrime) + 1) ):
                #if(dead & safePrime == dead):
                if (safePrime.bit_length() == bits):
                    break
        #safePrime = secrets.randbits(bits)
    ...
    while True:
        safePrime = secrets.randbits(bits)
        if (sympy.isprime(int(safePrime))) and (sympy.isprime( int((safePrime * 2) + 1) )) and (safePrime.bit_length() == bits):
            break
        #safePrime = secrets.randbits(bits)
...
def prim_root( N:int ) -> int:
    ...
    primeRoot = 0 # Will hold the prime root
    primFound = False
    for g in range(2,N-2): # N-2 so that N-1 is not considerred
        print("kill me pls, g: ", g, " primeRoot: ", primeRoot, " primFound: ", primFound)
        for i in range (len(q)): # if p mod g^((p-1)/q) != 1 for every element of q
            print((g**((N-1)/q[i])) % N)
            if (g**((N-1)/q[i])) % N != 1:
                primFound = False
                break
            primFound = True
        if primFound: #primeRoot has been found
          primeRoot = g  
